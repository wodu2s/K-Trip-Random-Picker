...
export default async function handler(req, res) {
  ...
  if (!key) {
    console.error("[kakao-navi] missing KAKAO_REST_API_KEY");
    res.status(500).json({ error: "KAKAO_REST_API_KEY 환경변수가 설정되지 않았습니다." });
    return;
  }
  ...
  try {
    ...
    console.error("[kakao-navi] fetch failed", message);
    res.status(502).json({ error: "Kakao 길찾기 요청 실패", message });
  }
}
